      ...
      <div className="flex-1 overflow-auto bg-muted/30 p-8">
        ...
        <div className="flex items-center justify-center min-h-full">
          ...
          <div
            ...
            className={`relative bg-white shadow-2xl ${isOver ? 'ring-4 ring-primary ring-opacity-50' : ''}`}
            style={{
            ...
            {canvas.grid.visible && (
              <div
                ...
                style={{
                  ...
                  `,
                  backgroundSize: `${canvas.grid.size * canvas.zoom}px ${canvas.grid.size * canvas.zoom}px`,
                }}
              />
            )}
            ...
            {canvas.guides.visible && (
              <>
                ...
                <div
                  ...
                  style={{ left: `${(template.canvas.width / 2) * canvas.zoom}px` }}
                />
                {/* Horizontal center */}
                <div
                  className="absolute left-0 right-0 h-px bg-blue-400/30 pointer-events-none"
                  style={{ top: `${(template.canvas.height / 2) * canvas.zoom}px` }}
                />
              </>
            )}
...
function CanvasElement({
  ...
  return (
    <Rnd
      ...
      onDragStop={(e, d) => {
        onMove(d.x / zoom, d.y / zoom);
      }}
      onResizeStop={(e, direction, ref, delta, position) => {
        const newWidth = parseInt(ref.style.width) / zoom;
        const newHeight = parseInt(ref.style.height) / zoom;
        onResize(newWidth, newHeight);
        onMove(position.x / zoom, position.y / zoom);
      }}
      onClick={onSelect}
...
function ElementContent({ element, zoom }: { element: VisualElement; zoom: number }) {
  const style = {
    ...element.style,
